...
class SaleController extends Controller
{
    ...
    //                             <li><a href="'.route('sale.lap_receipt', $sale->id).'" class="btn btn-link"><i class="fa fa-copy"></i> Lab Receipt</a></li>
    //                             <li>
    ...
    public function saleData(Request $request)
    {
        ...
        $data = array();
                ...
                foreach ($sales as $key=>$sale)
                {
                    ...
                    $nestedData['options'] = '<div class="btn-group">
                                ...
                                <ul class="dropdown-menu edit-options dropdown-menu-right dropdown-default" user="menu">
                                    <li><a href="'.route('sale.invoice', $sale->id).'" class="btn btn-link"><i class="fa fa-copy"></i> '.trans('file.Generate Invoice').'</a></li>
                                    <li><a href="'.route('sale.lap_receipt', $sale->id).'" class="btn btn-link"><i class="fa fa-copy"></i> Lab Receipt</a></li>
                                    <li><a href="'.route('sale.address_print', $sale->id).'" class="btn btn-link"><i class="fa fa-copy"></i> Address Print</a></li>
                                    <li>
                                        <button type="button" class="btn btn-link view"><i class="fa fa-eye"></i> '.trans('file.View').'</button>
                                    </li>';
                    ...
                    if(config('is_packing_slip') && in_array("packing_slip_challan", $all_permission) && ($sale->sale_status == 2 || $sale->sale_status == 5) ) {
                        $nestedData['options'] .=
                        '<li>
                            <button type="button" class="create-packing-slip-btn btn btn-link" data-id = "'.$sale->id.'" data-toggle="modal" data-target="#packing-slip-modal"><i class="dripicons-box"></i> '.trans('file.Create Packing Slip').'</button>
                        </li>';
            ...
            \Log::error('DataTables Error (Sale): ' . $e->getMessage(), ['exception' => $e]);
    ...
    public function store(Request $request)
    {
        ...
        try {*/
            ...
            // $latest_sale_q = Sale::select('reference_no')->orderBy('id', 'desc');

            // if($data['order_tax'] > 0){
            //     $data['reference_no'] = 'INV-';
            //     $latest_sale_q = $latest_sale_q->where('order_tax', '<>', 0);
            // }else{
            //     $latest_sale_q = $latest_sale_q->where('order_tax', false);
            //     $data['reference_no'] = 'EST-';
            // }

            // $latest_sale = $latest_sale_q->first();
            // if($latest_sale){
            //     $last_r = $latest_sale->reference_no;
            //     $last_e = explode('-', $last_r);
            //     $last_number = intval(array_pop($last_e)) + 1;
            // }else{
            //     $last_number = 1000;
            // }
            // $data['reference_no'] .= date("Ym").'-'. $last_number;


            // Order no
            $orderObj = Sale::select('order_no')->latest('id')->first();
            if($orderObj){
                $orderNr = $orderObj->order_no;
                $removedchar = substr($orderNr, 8);
                $order_number = str_pad($removedchar + 1, 5, "0", STR_PAD_LEFT);
            }else{
                $order_number  = str_pad(1000, 5, "0", STR_PAD_LEFT);
            }
            $data['order_no'] = 'LB-'.date("ym").'-'. $order_number;
            ...
            foreach ($product_id as $i => $id) {
                ...
                $product_sale['sale_price'] = $lims_product_data->price;
                ...
                $product_sale['sph'] = $sph[$i];
                $product_sale['cyl'] = $cyl[$i];
                $product_sale['axis'] = $axis[$i];
                $product_sale['lr'] = $lr[$i];
                $product_sale['base'] = $lims_product_data->base;
                $product_sale['addition'] = $addition[$i];
    ...
    public function getProduct(Request $request, $id)
    {
        ...
        // ->select('product_warehouse.*', 'products.name', 'products.code', 'products.type', 'products.product_list', 'products.qty_list', 'products.is_embeded', 'products.price')
        ...
        // ->select('product_warehouse.*', 'products.price','products.name', 'products.code', 'products.type', 'products.product_list', 'products.qty_list', 'products.is_embeded')
        // ->get();
        ...
        foreach ($lims_product_warehouse_data as $product_warehouse)
        {
            ...
            $product_type[] = $product_warehouse->productType->name;
            $product_category[] = $product_warehouse->category->name;
        ...
        //         $product_type[] = $lims_product_data->productType->name;
        ...
        // }
        // $product_data = [$product_code, $product_name, $product_category, $product_type, $product_qty, $product_code, $product_id, $product_list, $qty_list, $product_price];
        $product_data = [$product_name, $product_category, $product_type, $product_qty, $product_code, $product_id, $product_list, $qty_list, $product_price, $brand];
    ...
    public function limsProductSearch(Request $request)
    {
        ...
        $product_price = $lims_product_data->price;
        if($lims_product_data->is_variant){
            $product_price += $lims_product_data->additional_price;
        }
        ...
        foreach ($all_discount as $key => $discount) {
            ...
            if( ( $discount->applicable_for == 'All' || in_array($lims_product_data->id, $product_list) ) && ( $todayDate >= $discount->valid_from && $todayDate <= $discount->valid_till && in_array(date('D'), $days) && $qty >= $discount->minimum_qty && $qty <= $discount->maximum_qty ) ) {
                if($discount->type == 'flat') {
                    $product_price = $lims_product_data->price - $discount->value;
                }
                elseif($discount->type == 'percentage') {
                    $product_price = $lims_product_data->price - ($lims_product_data->price * ($discount->value/100));
                }
                $no_discount = 0;
                break;
            }
        ...
        if($lims_product_data->promotion && $todayDate <= $lims_product_data->last_date && $no_discount) {
            $product_price = $lims_product_data->promotion_price;
        }
        elseif($no_discount)
            $product_price = $lims_product_data->price;
        ...
        if($lims_product_data->tax_id) {
            ...
            $tax_rate = $lims_tax_data->rate;
            $tax_name = $lims_tax_data->name;
        }

        $unit_name_str = 'n/a,';
        $unit_operator_str = 'n/a,';
        $unit_operation_value_str = 'n/a,';
        ...
        if($lims_product_data->type == 'standard' || $lims_product_data->type == 'combo'){
            ...
            $unit_operator_str = implode(",",$unit_operator) . ',';
            $unit_operation_value_str = implode(",",$unit_operation_value) . ',';
        }

        // Return associative array for JS compatibility
        $response_data = [
            'name' => $lims_product_data->name,
            'code' => $product_variant_id ? $lims_product_data->item_code : $lims_product_data->code,
            'price' => $product_price,
            'tax_rate' => $tax_rate,
            'tax_name' => $tax_name,
            'tax_method' => $lims_product_data->tax_method,
            'unit_name' => $unit_name_str,
            'unit_operator' => $unit_operator_str,
            'unit_operation_value' => $unit_operation_value_str,
            'id' => $lims_product_data->id,
            'product_variant_id' => $product_variant_id,
            'promotion' => $lims_product_data->promotion,
            'is_batch' => $lims_product_data->is_batch,
            'is_imei' => $lims_product_data->is_imei,
            'is_variant' => $lims_product_data->is_variant,
            'qty' => $qty,
            ...
            'qty_list' => $lims_product_data->qty_list,
            // Numeric indices for backward compatibility
            0 => $lims_product_data->name,
            1 => $product_variant_id ? $lims_product_data->item_code : $lims_product_data->code,
            2 => $product_price,
            3 => $tax_rate,
            4 => $tax_name,
            5 => $lims_product_data->tax_method,
            6 => $unit_name_str,
            7 => $unit_operator_str,
            8 => $unit_operation_value_str,
            9 => $lims_product_data->id,
            10 => $product_variant_id,
            11 => $lims_product_data->promotion,
            12 => $lims_product_data->is_batch,
            13 => $lims_product_data->is_imei,
            14 => $lims_product_data->is_variant,
            15 => $qty,
    ...
    public function productSaleData($id)
    {
        ...
        foreach ($lims_product_sale_data as $key => $product_sale_data) {
            ...
            $product_sale[10][$key] = $product_sale_data->sph;
            $product_sale[11][$key] = $product_sale_data->cyl;
            $product_sale[12][$key] = $product_sale_data->axis;
            $product_sale[13][$key] = $product_sale_data->base;
            $product_sale[14][$key] = $product_sale_data->addition;
            $product_sale[15][$key] = $product_sale_data->lr;
    ...
    public function update(Request $request, $id)
    {
        ...
        $oldCustomerID = $lims_sale_data->customer_id;
        $oldOrderDate = $lims_sale_data->created_at;
        ...
        $lims_product_sale_data = Product_Sale::where('sale_id', $id)->get();
        // $data['created_at'] = date("Y-m-d", strtotime(str_replace("/", "-", $data['created_at']))) . ' '. date("H:i:s");
        $data['created_at'] = $lims_sale_data->created_at;
        $product_id = $data['product_id'];
        $product_batch_id = isset($data['product_batch_id']) ? $data['product_batch_id'] : "";
        $imei_number = isset($data['imei_number']) ? $data['imei_number'] : "";
        $product_code = $data['product_code'];
        $product_variant_id = isset($data['product_variant_id']) ?$data['product_variant_id'] : '';
        $qty = $data['qty'];
        ...
        foreach ($product_id as $key => $pro_id) {
            ...
            if($sale_unit[$key] != 'n/a') {
                ...
                if($data['sale_status'] == 1) {
                    ...
                    //deduct imei number if availablen
            ...
            $product_sale['discount_type'] = $discount_type[$key];
            $product_sale['discount_value'] = $discount_value[$key];
            $product_sale['tax_rate'] = $tax_rate[$key];
            $product_sale['tax'] = $tax[$key];
            $product_sale['total'] = $mail_data['total'][$key] = $total[$key];
            $product_sale['sph'] = $sph[$key];
            $product_sale['cyl'] = $cyl[$key];
            $product_sale['axis'] = $axis[$key];
            $product_sale['lr'] = $lr[$key];
            $product_sale['base'] = $lims_product_data->base;
            $product_sale['addition'] = $addition[$key];
        ...
        if($oldCustomerID != $lims_sale_data->customer_id){
            $billService->customerBillUpdate($oldOrderDate, $oldCustomerID);
        }else if($oldOrderDate != $lims_sale_data->created_at){
    ...
    public function genLabReceipt($id)
    {
        $lims_sale_data = Sale::find($id);
        $lims_product_sale_data = Product_Sale::where('sale_id', $id)->get();
        if(cache()->has('biller_list'))
    ...
    public function addressPrint($id)
    {
        $lims_sale_data = Sale::find($id);
        $lims_customer_data = Customer::find($lims_sale_data->customer_id);

        return view('backend.sale.address-print', compact('lims_sale_data', 'lims_customer_data'));
