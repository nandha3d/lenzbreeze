...
class ProductTypeController extends Controller
{
    ...
    public function data(Request $request)
    {
        ...
        $totalData = DB::table('product_types')->where('is_active', true)->count();
        ...
        if(empty($request->input('search.value')))
            $categories = ProductType::offset($start)
        ...
        else
        {
            ...
            $totalFiltered = ProductType::where([
                            ['name','LIKE',"%{$search}%"],
                            ['is_active', true]
                        ])->count();
        }
        ...
        if(!empty($categories))
        {
            foreach ($categories as $key=>$productType)
            {
                $nestedData['id'] = $productType->id;
                $nestedData['name'] = $productType->name;
                $nestedData['key'] = $key;
                ...
                $nestedData['options'] = '<div class="btn-group">
                            ...
                            <ul class="dropdown-menu edit-options dropdown-menu-right dropdown-default" user="menu">
                                <li>
                                    <button type="button" data-id="'.$productType->id.'" class="open-EditCategoryDialog btn btn-link" data-toggle="modal" data-target="#editModal" ><i class="dripicons-document-edit"></i> '.trans("file.edit").'</button>
                                </li>
                                <li class="divider"></li>'.
                                \Form::open(["route" => ["category.destroy", $productType->id], "method" => "DELETE"] ).'
                                <li>
    ...
    public function store(Request $request)
    {
        ...
        $productType = ProductType::create($lims_category_data);
    ...
    public function import(Request $request)
    {
        ...
        while($columns=fgetcsv($file))
        {
            ...
            $productType = ProductType::firstOrNew(['name' => $data['name'], 'is_active' => true ]);
            if($data['parentcategory']){
                $parent_category = ProductType::firstOrNew(['name' => $data['parentcategory'], 'is_active' => true ]);
    ...
    public function deleteBySelection(Request $request)
    {
        ...
        foreach ($category_id as $id) {
            ...
            $lims_category_data = ProductType::findOrFail($id);
