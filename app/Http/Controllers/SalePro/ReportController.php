...
class ReportController extends Controller
{
    ...
    public function productReportData(Request $request)
    {
        ...
        $getStats = function($model, $productIds, $warehouse_id, $start_date, $end_date, $unitCol) {
            ...
            return [
                ...
                'details' => $results->groupBy(function($item) {
                    return $item->product_id . '-' . ($item->variant_id ?? '0');
                })
            ];
        };
        ...
        // Purchase Stats
        $purchaseStats = $getStats('App\Models\ProductPurchase', $productIds, $warehouse_id, $start_date, $end_date, 'purchase_unit_id');
        // Sale Stats
        $saleStats = $getStats('App\Models\Product_Sale', $productIds, $warehouse_id, $start_date, $end_date, 'sale_unit_id');
        // Return Stats
        $returnStats = $getStats('App\Models\ProductReturn', $productIds, $warehouse_id, $start_date, $end_date, 'sale_unit_id');
        // Purchase Return Stats
        $purchaseReturnStats = $getStats('App\Models\PurchaseProductReturn', $productIds, $warehouse_id, $start_date, $end_date, 'purchase_unit_id');
        ...
        foreach ($lims_product_all as $product) {
            $processData = function($p, $vId = null, $itemCode = null) use (&$data, $units, $purchaseStats, $saleStats, $returnStats, $purchaseReturnStats, $warehouse_id, $productWarehouseMap, $variants) {
                $lookupKey = $p->id . '-' . ($vId ?? '0');
                $nestedData['key'] = count($data);
                
                if ($vId) {
                    $vName = $variants->get($vId)->name ?? '';
                    $nestedData['name'] = $p->name . ' [' . $vName . ']' . '<br>' . $itemCode;
                } else {
                    $nestedData['name'] = $p->name . '<br>' . $p->code;
                }
                
                $nestedData['category'] = $p->category->name;

                // Helper to calculate qty with units
                $calcQty = function($details, $key, $units, $unitCol) {
                    $total_qty = 0;
                    $rows = $details->get($key) ?? collect();
                    foreach ($rows as $row) {
                        $uId = $row->{$unitCol};
                        if (isset($units[$uId])) {
                            $unit = $units[$uId];
                            if ($unit->operator == '*') {
                                $total_qty += $row->qty * $unit->operation_value;
                ...
                $nestedData['purchased_amount'] = $purchaseStats['sums']->get($lookupKey) ?? 0;
                $nestedData['purchased_qty'] = $calcQty($purchaseStats['details'], $lookupKey, $units, 'purchase_unit_id');
                
                $nestedData['sold_amount'] = $saleStats['sums']->get($lookupKey) ?? 0;
                $nestedData['sold_qty'] = $calcQty($saleStats['details'], $lookupKey, $units, 'sale_unit_id');
                
                $nestedData['returned_amount'] = $returnStats['sums']->get($lookupKey) ?? 0;
                $nestedData['returned_qty'] = $calcQty($returnStats['details'], $lookupKey, $units, 'sale_unit_id');
                
                $nestedData['purchase_returned_amount'] = $purchaseReturnStats['sums']->get($lookupKey) ?? 0;
                $nestedData['purchase_returned_qty'] = $calcQty($purchaseReturnStats['details'], $lookupKey, $units, 'purchase_unit_id');
                ...
                if ($warehouse_id == 0) {
                    if ($vId) {
                        // For variants, we need the qty from ProductVariant
                        // This wasn't bulk fetched perfectly for the loop helper, but we have $productVariantsGrouped
                        // I'll just use the product's variant relationship if available or pre-fetch it
                        // Actually, I'll pass $pv to this function
                    } else {
                        $nestedData['in_stock'] = $p->qty;
                    }
                } else {
                    $nestedData['in_stock'] = isset($productWarehouseMap[$lookupKey]) ? $productWarehouseMap[$lookupKey]->first()->qty : 0;
                }

                $nestedData['profit'] = number_format((float)$nestedData['profit'], config('decimal'), '.', '');
                
                // We'll set in_stock and stock_worth outside this helper to handle variants/non-variants specifically
                return $nestedData;
            };

            if ($product->is_variant) {
                $pVariants = $productVariantsGrouped->get($product->id) ?? collect();
                foreach ($pVariants as $pv) {
                    $nestedData = $processData($product, $pv->variant_id, $pv->item_code);
                    if ($warehouse_id == 0) {
                        $nestedData['in_stock'] = $pv->qty;
                    }
                ...
                $nestedData = $processData($product);
                if (config('currency_position') == 'prefix')
                    $nestedData['stock_worth'] = config('currency').' '.($nestedData['in_stock'] * $product->price).' / '.config('currency').' '.($nestedData['in_stock'] * $product->cost);
                else
                    $nestedData['stock_worth'] = ($nestedData['in_stock'] * $product->price).' '.config('currency').' / '.($nestedData['in_stock'] * $product->cost).' '.config('currency');
    ...
    public function purchaseReportData(Request $request)
    {
        ...
        foreach ($lims_product_all as $product) {
            if ($product->is_variant) {
                $pVariants = $productVariantsGrouped->get($product->id) ?? collect();
                foreach ($pVariants as $pv) {
                    $vName = $variants->get($pv->variant_id)->name ?? '';
                    $nestedData['key'] = count($data);
                    $nestedData['name'] = $product->name . ' [' . $vName . ']' . '<br>' . $pv->item_code;
                    $nestedData['category'] = $product->category->name;

                    $lookupKey = $product->id . '-' . $pv->variant_id;
                    
                    // Purchased amount
                    $nestedData['purchased_amount'] = isset($purchaseSums[$lookupKey]) ? $purchaseSums[$lookupKey]->first()->total_purchased_amount : 0;

                    // Purchased qty calculation
                    $purchased_qty = 0;
                    $pPurchaseRows = $purchaseDetails->get($lookupKey) ?? collect();
                    foreach ($pPurchaseRows as $pp) {
                        if (isset($units[$pp->purchase_unit_id])) {
                            $unit = $units[$pp->purchase_unit_id];
                            if ($unit->operator == '*') {
                                $purchased_qty += $pp->qty * $unit->operation_value;
                            } elseif ($unit->operator == '/') {
                                $purchased_qty += $pp->qty / $unit->operation_value;
                            }
                        } else {
                            $purchased_qty += $pp->qty;
                        }
                    }
        ...
        foreach ($lims_product_all as $product) {
            if ($product->is_variant) {
                ...
                foreach ($pVariants as $pv) {
                    ...
                    // Sold amount
                    $nestedData['sold_amount'] = isset($saleSums[$lookupKey]) ? $saleSums[$lookupKey]->first()->total_sold_amount : 0;

                    // Sold qty calculation
                    $sold_qty = 0;
                    $pSaleRows = $saleDetails->get($lookupKey) ?? collect();
                    foreach ($pSaleRows as $ps) {
                        if (isset($units[$ps->sale_unit_id])) {
                            $unit = $units[$ps->sale_unit_id];
                            if ($unit->operator == '*') {
                                $sold_qty += $ps->qty * $unit->operation_value;
                            } elseif ($unit->operator == '/') {
                                $sold_qty += $ps->qty / $unit->operation_value;
                            }
                        } else {
    ...
    public function customerSaleData(Request $request)
    {
        ...
        $start = date('Y-m-01', strtotime($request->input('start_date')));
        $end = date('Y-m-t', strtotime($request->input('start_date')));

        $customer_id = $request->input('customer_id');
        $q = DB::table('customer_bills')
            ->whereDate('date', '>=' ,$start)
            ->whereDate('date', '<=' ,$end);

        if($request->input('customer_id') && $request->input('customer_id') != 'all'){
        ...
        $q = $q->select('id', 'reference_no', 'total_price', 'grand_total', 'paid_amount', 'date')
            ->offset($start)
            ->limit($limit)
            ->orderBy($order, $dir);
        ...
        if(!empty($sales))
        {
            foreach ($sales as $key => $sale)
            {
                $nestedData['id'] = $sale->id;
                $nestedData['key'] = $key;
                $nestedData['date'] = date(config('date_format'), strtotime($sale->date));
                $nestedData['reference_no'] = $sale->reference_no;
    ...
    public function customerSaleDataPrint(Request $request)
    {
        $start = date('Y-m-01', strtotime($request->input('start_date')));
        $end = date('Y-m-t', strtotime($request->input('start_date')));
        ...
        if($request->input('customer_id') && $request->input('customer_id') != 'all'){
            $a = $a->where('customer_id', '=', $request->input('customer_id'));
        }
        $a = $a->select(DB::raw('payment_reference as reference_no, 0 as grand_total, amount as paid_amount, date, customer_id, type_id as type '));
        ...
        $q = $q->select(DB::raw('reference_no, grand_total  , 0 as paid_amount,  date, customer_id, "sale" as type'))
                 ->union($a)
            ->orderBy('date');
        ...
        if(!empty($sales))
        {
            foreach ($sales as $key => $sale)
            {
                ...
                $nestedData['date'] = date(config('date_format'), strtotime($sale->date));
                $nestedData['reference_no'] = $sale->reference_no;
                $nestedData['type'] = $sale->type;

                $nestedData['grand_total'] = $sale->grand_total;
                $nestedData['paid'] = $sale->paid_amount;
                $nestedData['due'] = $sale->grand_total - $sale->paid_amount;

                $data[$sale->customer_id]['data'][] = $nestedData;
                $data[$sale->customer_id]['customer_id'] = $sale->customer_id;
                // $data[$sale->customer_id]['customer']['name'] = $sale->customer_name;
                // $data[$sale->customer_id]['customer']['city'] = $sale->customer_city;
                // $data[$sale->customer_id]['customer']['place'] = $sale->customer_place;
        ...
        return view('backend.report.customer_report_print',compact('data', 'start', 'end', 'year', 'month'));

    }

    public function gstExport(Request $request){
        $start = $request->input('start_date');
        return Excel::download(new GstExport($start), 'Gst_bill_report.xlsx');
    }
    ...
    public function customerGroupReport(Request $request)
    {
        ...
        return view('backend.report.customer_group_report',compact('start_date', 'end_date', 'customer_group_id', 'lims_customer_group_list'));
    }
    ...
    public function customerGroupSaleData(Request $request)
    {
        ...
        if($customer_group_id && $customer_group_id != 'all'){
            $q =  $q->where('customer_group_id', '=', $customer_group_id);
        }


        $totalData = $q->select(DB::raw('count(distinct cb.customer_id) as count'))->first()->count;
        ...
        $q = $q->select(DB::raw('customers.id as customer_id, customers.name as customer_name,customers.place as customer_place,
                        total_bill, total_price, open_bal, total_paid, bal_amount,
                         grand_total, total_payment, total_return'))
            ->offset($start)
            ->limit($limit)
            ->orderBy($order, $dir);
        ...
        if(!empty($sales))
        {
            foreach ($sales as $key => $sale)
            {
                ...
                $nestedData['customer'] = $sale->customer_name.' ['.($sale->customer_place).']';

                $nestedData['total_price'] = number_format($sale->total_price, cache()->get('general_setting')->decimal);
                $nestedData['grand_total'] = number_format($sale->grand_total, cache()->get('general_setting')->decimal);
                $nestedData['pre_bal'] = number_format($sale->open_bal, cache()->get('general_setting')->decimal);
                $nestedData['credit'] = number_format($sale->total_paid, cache()->get('general_setting')->decimal);
                $nestedData['payment'] = number_format($sale->total_payment, cache()->get('general_setting')->decimal);
                $nestedData['return'] = number_format($sale->total_return, cache()->get('general_setting')->decimal);

                $nestedData['cl_bal'] = number_format($sale->bal_amount , cache()->get('general_setting')->decimal);
    ...
    public function customerGroupReportPrint(Request $request)
    {
        $start_date = date('Y-m-01', strtotime($request->input('start_date')));
        $end_date = date('Y-m-t', strtotime($request->input('start_date')));
        $customer_group_id = $request->input('customer_group_id');
        ...
        $customer_group_name = '';
        if($customer_group_id && $customer_group_id != 'all'){
            $cg = CustomerGroup::select('name')->where('id', $customer_group_id)->first();
            if($cg){
        ...
        if(!empty($sales))
        {
            foreach ($sales as $key => $sale)
            {
                ...
                $nestedData['grand_total'] = $sale->grand_total;
                $nestedData['pre_bal'] = $sale->open_bal;
                $nestedData['credit'] = $sale->total_paid;
                $nestedData['cl_bal'] = $sale->bal_amount;
                $nestedData['customer_id']  = $sale->customer_id;
                $data[] = $nestedData;
            }
        }
...
//         print_r($data); exit;
        ...
        return view('backend.report.customer_group_report_print',compact('data', 'start', 'end', 'customer_group_name'));
