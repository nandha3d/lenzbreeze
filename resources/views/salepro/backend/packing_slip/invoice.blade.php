    ...
    <div>
        <table class="table table-bordered">
            <tbody>
                ...
                <tr>
                    <td>Order No: <strong>{{$sale->order_no}}</strong></td>
                    <td>Payment Status:
                    ...
                    <td colspan="2">
                        COD: <strong>BDT {{$sale->grand_total - $sale->paid_amount}}
                    </strong></td>
                </tr>
                <tr>
                ...
                <?php
                    $product = \App\Models\Product::select('name', 'code', 'base', 'addition')->find($packing_slip_product->product_id);
                    ...
                    $sale_product = \App\Models\Product_Sale::select('qty', 'total', 'sph', 'cyl', 'axis', 'lr')
                                    ->where([
                                        ['sale_id', $sale->id],
                                        ['product_id', $packing_slip_product->product_id]
                                    ])->first();
                ...
                <tr>
                    <td colspan="2">
                        ...
                        <span style="margin-left:25px">Base: {{$product->base}} </span>
                        <span style="margin-left:5px">Add: {{$product->addition}} </span>
                        <br>
                        <span>SPH: {{$sale_product->sph}} </span>
                        <span style="margin-left:5px">CYL: {{$sale_product->cyl}} </span>
                        <span style="margin-left:5px">AXIS: {{$sale_product->axis}} </span>
                        <span style="margin-left:5px">{{$sale_product->lr}} </span>
                        </td>
                ...
                <tr>
                    ...
                    <?php 
                        ...
                        echo '<img style="margin-top:10px;" src="data:image/png;base64,' . base64_encode($generator->getBarcode($sale->reference_no, $generator::TYPE_CODE_128)) . '" width="300" alt="barcode" />';
                    ?>
                    </td>
                </tr>
                @if($sale->sale_note)
